...
$paydate 	= $mb['mb_1'];									//결제날짜
$todate 	= date("Y-m-d H:i:s");							//오늘날짜
$DateDif 	= strtotime($todate) - strtotime($paydate) ;	//날짜차이
$Dday		= ceil($DateDif / (60*60 *24)) ; 				//일자변경
// 로그인 체크 
...
}else if($mb_level%2==1){ // 개인 로그인
	

}else if($mb_level%2==0){ //기업 로그인
	if( $Dday > 30 ){
//	 	alert('결제 기간이 종료되었습니다.');
//		$link = "/myPageComp.php";
	}
}
goto_url($link);
