...
//echo "<script>alert('lpay_form 1');</script>";
//
//echo "<script>alert('de_inicis_lpay_use ".$default['de_inicis_lpay_use']."');</script>";
//echo "<script>alert('de_inicis_kakaopay_use ".$default['de_inicis_kakaopay_use']."');</script>";
//echo "<script>alert('de_pg_service ".$default['de_pg_service']."');</script>";

이니시스 lpay 사용시에만 해당함
 if( ! ($default['de_inicis_lpay_use'] || $default['de_inicis_kakaopay_use']) || ('inicis' == $default['de_pg_service']) ){    //PG가 이니시스인 경우 아래 내용 사용 안함
     return;
 }
//echo "<script>alert('lpay_form 2');</script>";
