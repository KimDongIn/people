...
if($act == "buy")
{
    ...
    if ($is_member) // 회원인 경우
        goto_url(G5_SHOP_URL.'/orderform3.php');
    else
        goto_url(G5_BBS_URL.'/login.php?url='.urlencode(G5_SHOP_URL.'/orderform3.php'));
...
if ($sw_direct)
{
    if ($is_member)
    {
		echo "<script>alert('cartupdate3.php 이동');</script>";
    	goto_url(G5_SHOP_URL."/orderform3.php?sw_direct=$sw_direct");
    }
    else
    {
    	goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/orderform3.php?sw_direct=$sw_direct"));
    }
}
else
{
